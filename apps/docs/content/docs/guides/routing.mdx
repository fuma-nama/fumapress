...
You can access Fumapress runtime data from `ctx` prop, such as the content loader:
...
export default async function Page({ ctx }: { ctx: AppContext<typeof PressConfig.$context> }) {
