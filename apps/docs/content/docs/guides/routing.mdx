...
Fumapress supports creating routes from content sources, plugins, and local page files using the file-based routing model from [Waku](https://waku.gg).
...
You can access Fumapress runtime data from `getPressContext()`. Export it from your config with `config.utils()` so it is already typed to your app:
...
export default async function Page() {
  const { getLoader } = getPressContext();
