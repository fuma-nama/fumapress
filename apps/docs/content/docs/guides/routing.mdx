...
You can access Fumapress runtime data with `getPressContext()`, such as the content loader:

```tsx title="src/pages/sitemap-preview.tsx"
import { getPressContext } from "fumapress/router";
// config file
import type PressConfig from "../../press.config";

export default async function Page() {
  const ctx = getPressContext<typeof PressConfig.$context>();
