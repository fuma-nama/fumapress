...
Controls how a page becomes a feed item. Pages are dated with the creation date from your content source (e.g. the `date` frontmatter with Fumadocs MDX), falling back to the last modified date, **undated pages are excluded**, so are [fallback pages](/docs/internationalization#fallback-pages).
