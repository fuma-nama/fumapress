...
Cover images and author avatars render through the [`Image`](/docs/plugins/image) component. Paths under your own site work as they are, remote URLs need their hostname allowed by the image plugin:
...
export default defineConfig({
  // ...
}).plugins(imagePlugin({ domains: ["avatars.githubusercontent.com"] }));
```

<Callout type="warn" title="Watch for redirects">
