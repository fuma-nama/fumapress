...
The generated Wrangler config sets `html_handling: "drop-trailing-slash"`: `/docs/` redirects to `/docs`. URLs Fumapress generates for links, the sitemap and feeds have no trailing slash either, keep both in sync with `site.trailingSlash` if you change one.
