...
A plugin that creates its own pages registers one route per language with `localeRoutes()`, and fixes the `lang` prop of each copy with `withLang()`. They live in `fumapress/internal`, which is not covered by semver:
...
const plugin: PressPlugin = {
  ...
  createPages({ createPage }) {
    const routes = this.i18nConfig ? localeRoutes(this.i18nConfig) : [{ base: "" }];
    ...
    for (const { base, lang } of routes) {
      createPage({
        ...
        path: `${base}/changelog`,
        staticPaths: [],
        component: lang ? withLang(ChangelogPage, lang) : ChangelogPage,
      });
    }
  },
};
...
`base` is the language prefix, or the `(default)` route group for the language `hideLocale` hides. Registering a literal path per language keeps `staticPaths` free for the route's own dynamic segments.
