...
A plugin that creates its own pages registers one route per language with `localeRoutes()`, and fixes the `lang` prop of each copy with `withLang()`:

```tsx title="press.config.tsx"
import { localeRoutes, withLang, type PressPlugin } from "fumapress";
