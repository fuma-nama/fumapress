...
Internal links follow it too: links rendered with `Link` from `fumapress/client`, which includes navigation, blog and tag links, and site-root links in your content like `[Basics](/docs/basics)` or `[Basics](./basics.mdx)`. Links to files (a `.` in the last segment, like `/feed.xml`), external URLs, and relative links like `../basics` are kept as is.
