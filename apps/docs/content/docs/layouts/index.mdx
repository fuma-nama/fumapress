...
A layout is a server component, `renderPage` can return any JSX. Get the page body from the app context, `getPageBody` returns `{ node }` with the rendered content:
...
const config = defineConfig({
  // ...
  renderPage: (props) => <LegalPage {...props} />,
});

export const { getPressContext } = config.utils();
export default config;

async function LegalPage({ page }: { page: typeof config.$context.page }) {
  const body = await getPressContext().getPageBody(page);
  if (!body) throw new Error(`no adapter renders ${page.path}`);
...
The context also exposes `getPageToc`, `getPageLastModified`, and `defaultLayoutProps`, the pieces the built-in layouts are made of. A page that throws while rendering fails `fumapress build` with the route name, so a broken layout never ships as a blank page.
