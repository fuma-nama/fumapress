...
export interface FumapressNotionOptions {
  client: Client;
...
export function fumapressNotion({
  client,
  dataSourceId,
  blockConcurrency = 4,
}: FumapressNotionOptions): NotionIntegration {
  if (blockConcurrency < 1 || !Number.isInteger(blockConcurrency)) {
    throw new TypeError("[@fumapress/notion] blockConcurrency must be a positive integer");
  }
        ...
        async files() {
          ...
          do {
            ...
            if (response.request_status?.type === "incomplete") {
              throw new Error(
                "[@fumapress/notion] The data source query reached Notion's result limit. Narrow the query filter so every page can be loaded.",
              );
            }
          ...
          return Promise.all(
            pages.map(async (page) => {
              ...
              if (duplicate) {
                throw new Error(
                  `[@fumapress/notion] Pages "${duplicate}" and "${page.id}" resolve to the same virtual path: ${filePath}`,
                );
              }
...
function parseSlug(value: string, pageId: string): string[] {
  ...
  if (slugs.some((part) => part === "." || part === "..")) {
    throw new Error(`[@fumapress/notion] Page "${pageId}" has an unsafe slug: ${value}`);
  }
  return slugs;
}
...
function resolveVirtualPath(baseDir: string | undefined, generatedPath: string): string {
  ...
  if (!normalized || normalized.split("/").some((part) => part === "..")) {
    throw new Error(`[@fumapress/notion] Invalid virtual path: ${generatedPath}`);
  }
