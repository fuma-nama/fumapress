...
import { fumapressNotion } from "../src/source";
import { block, dataSourceId, page, richText } from "./fixtures";

describe("fumapressNotion", () => {
  it("queries every page and lazily loads paginated nested blocks once", async () => {
    ...
    const integration = fumapressNotion({ client, dataSourceId });
  ...
  it("rejects duplicate virtual paths", async () => {
    ...
    await expect(fumapressNotion({ client, dataSourceId }).dynamicSource().files()).rejects.toThrow(
  ...
  it("limits nested block requests across sibling branches", async () => {
    ...
    await fumapressNotion({ client, dataSourceId, blockConcurrency: 1 }).getBlocks(page().id);
  ...
  it("hydrates meeting note sections referenced by block ID", async () => {
    ...
    await expect(
      fumapressNotion({ client, dataSourceId }).getBlocks(page().id),
