...
With i18n, pages inherited from the fallback language (their `absolutePath` is the source page's) get `noindex` and a canonical link to the source page, and the sitemap, RSS feed, `llms-full.txt`, and Takumi image generation skip them. They stay in the search index, so readers of that locale still find the content.
